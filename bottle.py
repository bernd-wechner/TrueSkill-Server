...
__version__ = '0.12.16'        # PATCHED VERSION to work around lighttpd bug: https://redmine.lighttpd.net/issues/3005
__license__ = 'MIT'
...
class Bottle(object):
    ...
    def _handle(self, environ):
        path = environ['bottle.raw_path'] = environ.get('PATH_INFO',environ.get('SCRIPT_NAME','/'))
        if py3k:
            try:
...
def run(app=None, server='wsgiref', host='127.0.0.1', port=8080,
    ...
    try:
        ...
        if not server.quiet:
            _stderr("PATCHED Bottle v%s server starting up (using %s)...\n" % (__version__, repr(server)))
